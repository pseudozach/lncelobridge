...
// @title Hash timelock contract for CELO
contract EtherSwap {
    // State variables
    ...
    /// @dev Mapping between value hashes of swaps and whether they have CELO locked in the contract
    ...
    /// Locks CELO for a swap in the contract
    /// @notice The amount locked is the CELO sent in the transaction and the refund address is the sender of the transaction
    /// @param preimageHash Preimage hash of the swap
    /// @param claimAddress Address that can claim the locked CELO
    /// @param timelock Block height after which the locked CELO can be refunded
    function lock(
    ...
    /// Locks CELO for a swap in the contract and forwards a specified amount of CELO to the claim address
    /// @notice The amount locked is the CELO sent in the transaction minus the prepay amount and the refund address is the sender of the transaction
    /// @dev Make sure to set a reasonable gas limit for calling this function, else a malicious contract at the claim address could drain your CELO
    /// @param preimageHash Preimage hash of the swap
    /// @param claimAddress Address that can claim the locked CELO
    /// @param timelock Block height after which the locked CELO can be refunded
    ...
    function lockPrepayMinerfee(
        ...
        // Lock the amount of CELO sent minus the prepay amount in the contract
    ...
    /// Claims CELO locked in the contract
    ...
    /// @param refundAddress Address that locked the CELO in the contract
    /// @param timelock Block height after which the locked CELO can be refunded
    function claim(
        bytes32 preimage,
        uint amount,
    ...
    /// @param claimAddress Address that can claim the locked CELO
    /// @param refundAddress Address that locked the CELO and can refund them
    /// @param timelock Block height after which the locked CELO can be refunded
    ...
    /// Checks whether a swap has CELO locked in the contract
    /// @dev This function reverts if the swap has no CELO locked in the contract
    /// @param hash Value hash of the swap
    function checkSwapIsLocked(bytes32 hash) private view {
        require(swaps[hash] == true, "EtherSwap: swap has no CELO locked in the contract");
    }
}