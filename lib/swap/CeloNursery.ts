...
class CeloNursery extends EventEmitter {
  private celoManager: CeloManager;
  ...
  constructor(
    ...
    this.celoManager = walletManager.celoManager!;

    this.listenBlocks();

    this.logger.info("CeloNursery listeners are starting...");
  ...
  public init = async (): Promise<void> => {
    ...
    for (const mempoolReverseSwap of mempoolReverseSwaps) {
      ...
      // Skip all Reverse Swaps that didn't send coins on the Celo chain
      if (this.getEthereumWallet(chainCurrency) === undefined) {
        this.logger.error("CeloNursery Skip all Reverse Swaps that didn't send coins on the Celo chain");
        continue;
      }

      try {
        const transaction = await this.celoManager.provider.getTransaction(mempoolReverseSwap.transactionId!);
  ...
  private listenEtherSwap = () => {
    this.celoManager.contractEventHandler.on('eth.lockup', async (
      transactionHash,
      etherSwapValues,
    ) => {
      ...
      this.logger.verbose("CeloNursery.154 listenetherswap claimAddress, this.celomanageraddress: " + etherSwapValues.claimAddress + ", " + this.celoManager.address + ", " + JSON.stringify(etherSwapValues));
      if (etherSwapValues.claimAddress !== this.celoManager.address) {
        this.emit(
          'lockup.failed',
          swap,
          Errors.INVALID_CLAIM_ADDRESS(etherSwapValues.claimAddress, this.celoManager.address).message,
        );
        return;
      }
    ...
    this.celoManager.contractEventHandler.on('eth.claim', async (transactionHash, preimageHash, preimage) => {
  ...
  private listenERC20Swap = () => {
    this.celoManager.contractEventHandler.on('erc20.lockup', async (
      transactionHash,
      erc20SwapValues,
    ) => {
      let swap = await this.swapRepository.getSwap({
      ...
      if (erc20SwapValues.claimAddress !== this.celoManager.address) {
        this.emit(
          'lockup.failed',
          swap,
          Errors.INVALID_CLAIM_ADDRESS(erc20SwapValues.claimAddress, this.celoManager.address).message,
        );
        return;
      }
    ...
    this.celoManager.contractEventHandler.on('erc20.claim', async (transactionHash, preimageHash, preimage) => {
      const reverseSwap = await this.reverseSwapRepository.getReverseSwap({
        preimageHash: {
          [Op.eq]: getHexString(preimageHash),
        },
  ...
  private listenBlocks = () => {
    this.celoManager.provider.on('block', async (height) => {
      // this.logger.error("CeloNursery on block: " + height.toString());
      await Promise.all([
